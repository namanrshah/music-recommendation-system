 ...
 * @author namanrs
 */
public class ReducerUserCandidateSongsTwo extends
    ...
    protected void reduce(Text key, Iterable<Text> values,
        ...
        if (!candidateSongs.isEmpty() && listenedSongsList != null) {
            ...
            if (size > 0) {
                for (String candidateSong : candidateSongs) {
                    strToWrite.append(candidateSong + Constants.SEPARATORS.COMBO_SEPARATOR);
                }
                context.write(new Text(Constants.STARTS_WITH.CANDIDATE_SONGS_FOR_USER + key), new Text(strToWrite.substring(0, strToWrite.length() - 1)));
            }
        }
    }
}
